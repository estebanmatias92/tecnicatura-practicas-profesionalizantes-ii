...
# Acceder a MySQL
# NOTA: La contraseña es 'rootpassword' (NO es 'samplevault' que es el nombre de la DB)
docker exec -it samplevault-db mysql -u root -prootpassword
```
...
1. Crear la base de datos y usuario:
   * **DB root password:** `rootpassword` (ver en `config/db/.env`)
   * **Usuario app:** `samplevault` / `samplevault`
   * **Nombre DB:** `samplevault`
...
1. Ejecutar el script de inicialización:
   * **NOTA:** `samplevault` es el nombre de la base de datos, NO la contraseña

```bash
mysql -u root -prootpassword samplevault < backend/config/init.sql
```
