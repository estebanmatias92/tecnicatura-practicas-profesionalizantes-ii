...
### Backend
- [ ] En `authController.register`, antes de `bcrypt.hash(password, 10)`, se valida que `password.length >= 8`
- [ ] Si la contraseña es menor a 8 caracteres, responde con HTTP `400` y `{ message: "La contraseña debe tener al menos 8 caracteres." }`
- [ ] No se ejecuta `bcrypt.hash` si la validación falla
- [ ] No se ejecuta la consulta a la DB si la validación falla

### Frontend
- [ ] El formulario de registro muestra un modal/mensaje con "La contraseña debe tener al menos 8 caracteres."
- [ ] El mensaje es específico para este error (no un genérico "Error en el registro")
- [ ] Los campos del formulario no se pierden al mostrar el error

### Tests
- [ ] **Test positivo:** Registrar con contraseña de 8 caracteres → espera `201`
- [ ] **Test negativo:** Registrar con contraseña de 7 caracteres → espera `400` + mensaje específico
- [ ] **Test de borde:** Registrar con contraseña vacía → espera `400` (por la validación de presencia existente)
- [ ] **Test de borde:** Registrar con contraseña de exactamente 8 caracteres → espera `201`
